...
class ESS_MSEB(nn.Module):
    ...
    def forward(self, x_in, mask=None):
        ...
        x0_out = self.atten(x0, mask=mask)
        x1_out = self.atten(x1, mask=mask)
        x2_out = self.atten(x2, mask=mask)
        x3_out = self.atten(x3, mask=mask)
...
class ESST_NMM(nn.Module):
    def __init__(self, dim=28, stage=3, num_blocks=[2,2,2]):
        super(ESST_NMM, self).__init__()
        self.dim = dim
        self.stage = stage
