...
# from draw_feature import draw_features
...
def draw_features(x):
    ...
    for i in range(10):
        ...
        plt.imshow(img)
        # print("{}/{}".format(i, width * height))
        savename = f'/data/users/zhaowangcai01/MST-main/figure/test{i}_inter_plus.png'
        plt.savefig(savename,bbox_inches='tight',dpi=100,pad_inches=0.0)
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=28, n_feat=28, stage=3):
        ...
        modules_body = [MST(dim=n_feat, stage=2, num_blocks=[2,2,2]) for _ in range(stage)]
    ...
    def forward(self, y, Phi=None):
        ...
        h = self.conv_out(h)
        # h_1 = h.detach().cpu().numpy().astype(np.float32)
        # draw_features(h_1)
        # h_1 = h.detach().cpu().numpy().astype(np.float32)
        # scio.savemat('mst_plus_noregion_2.mat', {'pred': h_1})
        # draw_features(h.cpu().numpy(),'/data/users/zhaowangcai01/MST-main/figure/test2_mst_pls.png')
        h += x
