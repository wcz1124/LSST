...
class LSSTM_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=28, n_feat=28, stage=3):
        super(LSSTM_Plus_Plus, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False)
        modules_body = [LSSTM(dim=n_feat, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]
...
class ESS_BL(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 3, 1)
        # lcr = self.lcb(x)
        for (lcb,attn1,attn2,ff2,fu) in self.blocks:
            lcr = lcb(x)
            x = attn1(x) + x
            # x = ff1(x) + x
            x = attn2(x) + x
...
class LSSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # Encoder
        fea_encoder = []
        for (ESS_BL, FeaDownSample,Feasw) in self.encoder_layers:
            fea = ESS_BL(fea)
...
class CBlock(nn.Module):
    ...
    def __init__(self, dim):
        ...
        # self.mask = nn.Conv2d(1, dim, 1, 1, bias=False)
