...
def gen_meas_torch(data_batch, mask3d_batch, Y2H=True, mul_mask=False):
    ...
    # input = add_gaussian_noise(input,0.01)
    # input = torch.FloatTensor(input.copy())
    # input = input.cuda()
    meas = input
    # meas = add_poisson_noise(meas,20)
    if Y2H:
        meas = meas / nC * 2
        H = shift_back(meas)
        if mul_mask:
...
def checkpoint_best(model, epoch, model_path, logger):
    model_out_path = model_path + "/model_epoch_best.pth"
    torch.save(model.state_dict(), model_out_path)
    logger.info("Checkpoint saved to {}".format(model_out_path))



def LoadTraining_Hd(path):
    imgs = []
    scene_list = os.listdir(path)
    scene_list.sort()
    print('training sences:', len(scene_list))
...
def LoadTest_ICVL(path_test):
    ...
    for i in range(len(scene_list)):
        ...
        img = np.transpose(img,(1,2,0))
        img = img[:1000,:1000,1:29]
        test_data[i, :, :, :] = img[372:628,372:628,:]
    test_data = torch.from_numpy(np.transpose(test_data, (0, 3, 1, 2)))
    return test_data

def LoadTest_Hd(path_test):
    scene_list = os.listdir(path_test)
    scene_list.sort()
    test_data = np.zeros((len(scene_list), 256, 256, 28))
    for i in range(len(scene_list)):
        ...
        img = (img - np.min(img))/(np.max(img)-np.min(img))
        test_data[i, :, :, :] = img[648:904,568:824,1:29]
    test_data = torch.from_numpy(np.transpose(test_data, (0, 3, 1, 2)))
    return test_data
...
#     """
#     Add Poisson noise to the input image with a specified signal-to-noise ratio (SNR).
...
#     # Calculate the noise power based on the specified SNR
#     noise_power = signal_power / (10 ** (snr / 10))

#     # Generate Poisson noise with the calculated noise power
#     noise = np.random.poisson(lam=noise_power, size=image.shape)
...
def add_poisson_noise(batch_image, snr):
    ...
    batch_image_noisy = torch.rand(b,h,w).cuda()
    for i in range(b):
        image = batch_image[i,:,:]
        ...
        noise_power  = int(noise_power*255)
        # Generate Poisson noise with the calculated noise power
        ...
        # 生成服从泊松分布的随机数
        # lam = torch.tensor(5.0)  # 泊松分布的参数 lambda
        # noise = torch.poisson(noise_power, size=(c,h,w))
        noise = torch.from_numpy(noise).cuda()
        # Add noise to the image
        noisy_image = image + noise
        batch_image_noisy[i,:,:]=noisy_image

    return batch_image_noisy
...
# 绘制原始图像和添加高斯噪声后的图像
fig, axes = plt.subplots(1, len(sigmas) + 1, figsize=(15, 5))
