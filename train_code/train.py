...
from architecture.focal_region_loss import FocalSpectralLoss
# from architecture.focal_region_loss import FocalRegionLoss
...
criterion = FocalSpectralLoss(loss_weight=1,alpha=opt.alpha,log_matrix=True).cuda()
# criterion = FocalRegionLoss(loss_weight=1,alpha=opt.alpha,log_matrix=True).cuda()
def train(epoch, logger):
    ...
    for i in range(batch_num):
        ...
        if opt.method in ['cst_s', 'cst_m', 'cst_l']:
            ...
            model_out_edge = spectral_edge(model_out)
            gt_edge = spectral_edge(gt)
            
            if opt.loss_type=='ffl':
                loss = criterion(model_out, gt) + 0.5*torch.sqrt(mse(model_out_edge, gt_edge))
            else:
                loss = torch.sqrt(mse(model_out, gt))
...
def main():
    ...
    logger.info("Learning rate:{}, batch_size:{},gpu_id:{}.\n".format(opt.learning_rate, opt.batch_size,opt.gpu_id))
    psnr_max = 0
    for epoch in range(1, opt.max_epoch + 1):
        ...
        if psnr_mean > psnr_max:
            ...
            if psnr_mean > 20:
                ...
                checkpoint_best(model, epoch, model_path, logger)
