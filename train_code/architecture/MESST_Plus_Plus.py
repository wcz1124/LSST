...
class MESST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=28, n_feat=28, stage=3):
        super(MESST_Plus_Plus, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False)
        modules_body = [MESST(dim=n_feat, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]
        self.fution = nn.Conv2d(56, 28, 1, padding=0, bias=True)
