...
class ESS_MSEB(nn.Module):
    def __init__(
        ...
        self.atten_bo = MS_MSA(dim=10, dim_head=10, heads=self.num_heads)
...
class ESST_Plus_Plus_Hd(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, stage=3):
        super(ESST_Plus_Plus_Hd, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False)
        modules_body = [ESST(dim=n_feat, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]
        self.fution = nn.Conv2d(62, 31, 1, padding=0, bias=True)
